...
def _find_repo_root(start: Path) -> Path:
    for candidate in [start, *start.parents]:
        if (candidate / "genetic_solver_components_new.py").exists() and (candidate / "data").exists():
            return candidate
...
import genetic_solver_components_new as core_solver
...
class MultiDaySandboxRunner:
    ...
    def solve_day(self, day_payload: dict[str, Any], *, render_map: bool) -> DayResult:
        ...
        routes, states, unassigned = core_solver.assign_tasks_genetic(dataset, graph, day_payload, cache)
