...
def generate_tasks_lp(
    ...
    try:
        ...
        return generate_tasks_greedy(dataset, graph, cache, show_progress=show_progress)
    ...
    tasks: list[Task] = []
    tid = 0
    for i, src in enumerate(_progress(
        sources,
        enabled=show_progress,
        ...
        for j, snk in enumerate(sinks):
            ...
            if flow > EPS:
                ...
                tid += 1
    return tasks if tasks else generate_tasks_greedy(dataset, graph, cache, show_progress=show_progress)
...
def generate_tasks_greedy(
    ...
    tasks: list[Task] = []
    for tid, src in enumerate(_progress(
        sources,
        enabled=show_progress,
...
def run_gap_vrp(
    ...
    if verbose:
        print("\n[Шаг 1] Формирование задач...")
    if step1_method == "lp":
        tasks = generate_tasks_lp(dataset, graph, cache, show_progress=show_progress)
    else:
        tasks = generate_tasks_greedy(dataset, graph, cache, show_progress=show_progress)
