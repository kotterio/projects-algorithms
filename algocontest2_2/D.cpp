...
class SparseTable {
 public:
  explicit SparseTable(std::vector<int> &arr) {
    ...
    gcd_table_.assign(n_, std::vector<int>(1));
    for (int i = 0; i < n_; ++i) {
      // sum_table[i][0] = arr[i];
      gcd_table_[i][0] = arr[i];
    }
    for (int i = 0; i < n_; ++i) {
      for (int j = 1; j < static_cast<int>(power2_.size()) && i + power2_[j] - 1 < n_; ++j) {
        // sum_table[i].emplace_back(0);
        gcd_table_[i].emplace_back(0);
      }
    }
    for (int i = n_ - 1; i >= 0; --i) {
      for (int j = 1; j < static_cast<int>(power2_.size()) && i + power2_[j] - 1 < n_; ++j) {
        // sum_table[i][j] = sum_table[i][j - 1] + sum_table[i + power2[j - 1]][j - 1];
        gcd_table_[i][j] = std::gcd(gcd_table_[i][j - 1], gcd_table_[i + power2_[j - 1]][j - 1]);
      }
    }
  }
  int GcdQuery(int l, int r) {
    return std::gcd(gcd_table_[l][power_[r - l + 1]],
                    gcd_table_[r + 1 - power2_[power_[r - l + 1]]][power_[r - l + 1]]);
  }

 private:
  int n_;
  // std::vector <std::vector <int>> sum_table;
  std::vector<std::vector<int>> gcd_table_;
...
int main() {
  ...
  for (size_t i = 0; i < q; i++) {
    ...
    std::cin >> r;
    r--;
    std::cout << table.GcdQuery(l, r) << std::endl;
  }
}